...
describes what users get. Pick a format with `-o`, or let jolpiaf1 choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
jolpiaf1 <command> -o table   # aligned columns for reading
jolpiaf1 <command> -o jsonl   # one JSON object per line, for piping
jolpiaf1 <command> -o json    # a single JSON array
jolpiaf1 <command> -o csv     # spreadsheet friendly
jolpiaf1 <command> -o tsv     # tab-separated
jolpiaf1 <command> -o url     # just the URL column
jolpiaf1 <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
jolpiaf1 <command> --fields id,title,url
```
...
```bash
jolpiaf1 <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
jolpiaf1 <command>            # a table, because this is a terminal
jolpiaf1 <command> | wc -l    # JSONL, because this is a pipe
```
